...
Kogoro's `artwork`, `subtitle`, and `metadata` subcommands process files sequentially. The `scan` command already supports concurrent processing via `Scanner.scanBatch()`.
